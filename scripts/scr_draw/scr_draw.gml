...
for (i=0<num,i+1); //num = amount of cards to draw stated in obj_deck_gen
{
	...
	{
		var card_draw=ds_list_find_value(deck_p,0); // draw cards from the top of the deck 
		if (hand_p[1]==noone)
		{
			hand_p[1]=instance_create_layer(50,100,"instance",obj_card_p);
			hand_p[1].sprite_index=card_draw;
			with (hand_p[1]){scr_assign_abilities();}
		}
		else if (hand_p[2]==noone)
		{
			hand_p[2]=instance_create_layer(50,140,"instance",obj_card_p);
			hand_p[2].sprite_index=card_draw;
			with (hand_p[2]){scr_assign_abilities();}
		}
		else if (hand_p[3]==noone)
		{
			hand_p[3]=instance_create_layer(50,180,"instance",obj_card_p);
			hand_p[3].sprite_index=card_draw;
			with (hand_p[3]){scr_assign_abilities();}
		}
		else if (hand_p[4]==noone)
		{
			hand_p[4]=instance_create_layer(50,240,"instance",obj_card_p);
			hand_p[4].sprite_index=card_draw;
			with (hand_p[4]){scr_assign_abilities();}
		}
	}
}
